...
L42100:
    if (qempty_(prsvec_1.prso) || 
	    (objcts_1.oflag1[prsvec_1.prso - 1] & TAKEBT) == 0) {
        goto L10;
    }
...
L60000:
    if ((objcts_1.oflag1[prsvec_1.prsi - 1] & FLAMBT + 
        LITEBT + ONBT) != FLAMBT + LITEBT + ONBT) {
        goto L60400;
    }
    if (objact_()) {
        return ret_val;
...
L83000:
    if (objcts_1.oroom[oindex_1.pump - 1] == play_1.here || 
	    objcts_1.oadv[oindex_1.pump - 1] == play_1.winner) {
        goto L83100;
    }
    rspeak_(632);
    /* !NO. */
...
L87000:
    ...
    /* !VARIETY OF JOKES. */
    if (! f && (prsvec_1.prso == oindex_1.wall || 
	    prsvec_1.prso >= oindex_1.wnort && 
		prsvec_1.prso <= oindex_1.wnort + 3)) {
        i = 656;
    }
    rspeak_(i);
    /* !JOKE. */
