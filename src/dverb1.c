...
L600:
    if (x != 0 && objcts_1.oadv[x - 1] == play_1.winner || 
	    weight_(0,prsvec_1.prso, play_1.winner) + 
		objcts_1.osize[prsvec_1.prso - 1] <= state_1.mxload) {
        goto L700;
    }
    rspeak_(558);
    /*  !TOO MUCH WEIGHT. */
...
L500:
    ...
    /*  !ALREADY INSIDE. */
    rspsb2_(564, objcts_1.odesc2[prsvec_1.prso - 1], 
	    objcts_1.odesc2[prsvec_1.prsi - 1]);
    ret_val = TRUE_;
    return ret_val;

L600:
    if (weight_(0, prsvec_1.prso, 0) + weight_(0, prsvec_1.prsi,0) + 
	    objcts_1.osize[prsvec_1.prso - 1] <= 
