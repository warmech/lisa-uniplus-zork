...
void fightd_()
{
    ...
    for (i = 1; i <= i__1; ++i) {
        ...
        objcts_1.ocapac[obj - 1] = (i__2 = objcts_1.ocapac[obj - 1], 
		    abs(i__2));
        vill_1.vprob[i - 1] = 0;
        if (ra == 0) {
            goto L2400;
...
L100:
    ...
    for (i = 1; i <= i__1; ++i) {
    /* !SEARCH VILLAIN. */
        if (objcts_1.ocan[i - 1] == v && 
		    (objcts_1.oflag2[i - 1] & WEAPBT) != 0) {
            dweap = i;
        }
    /* L200: */
    }
...
integer r;
{
    ...
    if (! findex_1.endgmf) {
        ...
        ret_val = r == rindex_1.mrg || r == rindex_1.mrge || 
		    r == rindex_1.mrgw || r == rindex_1.inmir && 
